...
class DiyTransformer(nn.Module):
    def __init__(self, vocab_size, embed_dim, num_hidden_layers=12, hidden_size=768, num_attention_heads=12):
        super().__init__()
        self.embed = nn.Embedding(vocab_size, embed_dim)
        # 投影层：确保 embed_dim -> hidden_size
        self.embed_proj = nn.Linear(embed_dim, hidden_size) if embed_dim != hidden_size else nn.Identity()
        self.layers = nn.ModuleList([
            MyLayer(hidden_size=hidden_size, num_attention_heads=num_attention_heads) for _ in range(num_hidden_layers)
        ])
        # 输出层：将 hidden_size 映射到词表大小
        self.lm_head = nn.Linear(hidden_size, vocab_size)

    def forward(self, x, attention_mask=None):
        x = self.embed(x)  # # [B, S, embed_dim]
        x = self.embed_proj(x)  # [B, S, hidden_size]
